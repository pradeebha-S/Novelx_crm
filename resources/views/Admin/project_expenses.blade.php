...
<ul class="nav nav-pills flex-column flex-sm-row mb-4 gap-sm-0 gap-2 mt-4">
    <li class="nav-item">
        <a class="nav-link  waves-effect waves-light" href="{{ route('admin.common_expenses') }}">Common
            Expenses</a>
    </li>
    <li class="nav-item">
        <a class="nav-link active waves-effect waves-light" href="{{ route('admin.project_expenses') }}">Project
            Expenses</a>
    </li>
</ul>
<div class="card p-3">
    ...
    <form action="{{route('add_project_expenses')}}" method="POST" id="login_form" enctype="multipart/form-data">
        @csrf
        <div class="row">
...
$(function() {
    ...
    $('#dept').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('project_expenses_data') }}",

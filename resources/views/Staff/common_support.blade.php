...
<div class="card p-4 mt-4">
    <h6>Common Support</h6>
    <form action="{{ route('add_support') }}" method="post" id="login_form">
        @csrf
        <div class="row">
            <div class="col-lg-6 mb-2">
                <label class="form-label">Title</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror"
                    placeholder="Enter common support title" name="title" value="{{ old('title') }}">
...
<script>
   ...
   $(function () {
    $('#dept').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('common_support_data') }}",
